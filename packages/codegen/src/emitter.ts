...
function emitRuntimeCommand({
  ...
  return `{ path: '${pathString}', paramNames: ${emitParamNames(cmd.paramNames)}${desc}${hidden}, load: ${emitLoadFn({ cmd, eager })} }`;
}
...
export function emitGeneratedFile({
  ...
  // The registry block references every command by `typeof X.options` /
  // `typeof X.params`, so every command needs a type-only import in lazy mode
  // (eager mode also needs them as values for the load thunks).
  const lines: string[] = [];
  lines.push(
  ...
  const typeImports: string[] = ['InferForwardedOptions', 'RuntimeNode'];
  if (usesInferParams) {
