...
- [`references/testing.md`](references/testing.md) — using `cli.run(argv)` (not `main()`) for integration tests, the `@parshjs/core/testing` submodule, injecting test doubles through `context`, capturing stdout/stderr, and what not to test.
